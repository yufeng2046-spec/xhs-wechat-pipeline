...
**卡片6 - 行动总结**: 3-5条行动清单(checklist格式) + 结尾引导语（自然收束，禁止诱导点赞/收藏/评论）
...
**重要：小红书社区规范禁止引导用户点赞/收藏/评论。CTA必须是价值导向的收束（如"今天就开始行动"），绝对不能出现"点赞""收藏""评论""关注"等诱导性字词。**
...
{{
  ...
  "summary": {{"title": "总结标题(≤20字)", "checklist": ["行动项1", "行动项2", "行动项3"], "cta": "结尾引导语（自然收束，如'今天就可以试试'，禁止诱导互动字词）"}}
}}"""
...
def _fallback_transform(script: Script) -> dict:
    ...
    return {
        ...
        "summary": {"title": "今日行动", "checklist": checklist[:4], "cta": "今天就可以试试，做起来才有效果"},
    }
